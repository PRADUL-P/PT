...
function setupRevitBridge() {
    ...
    function registerListener() {
        if (window._revitBridgeRegistered) return;  // prevent double-registration
        window._revitBridgeRegistered = true;
    ...
    // Try to register immediately (works if page is already in WebView2 context)
    if (typeof window.chrome !== 'undefined' && typeof window.chrome.webview !== 'undefined') {
        registerListener();
    } else {
        // Fallback: poll briefly, then also listen for the hostObjectsReady event
        let attempts = 0;
        const checkInterval = setInterval(() => {
            ...
            if (typeof window.chrome !== 'undefined' && typeof window.chrome.webview !== 'undefined') {
                ...
                registerListener();
            } else if (attempts >= 150) {  // 15-second maximum (150 * 100ms)
                clearInterval(checkInterval);
            }
        }, 100);
    }
